...
local icondir = usr.icon_dir .. 'powermenu/'
...
return function(s)
  ...
  local profile_name = wibox.widget {
    ...
    valign = 'center',
    text = " ",
    font = usr.font.bold,  -- 30 pt,
    widget = wibox.widget.textbox
  }

  -- Get user's profile script from /var/lib/AccountsService/icons/${USER}
  ...
  local update_profile_picture = function()
    awful.spawn.easy_async_with_shell(
      usr.scripts .. "pfp.sh 'userPfp'",
      function(stdout)
          local img
        if stdout then
          ing = stdout:gsub("\n", "")
          img = icondir .. "defaultpfp.svg"		-- FIXME
        else
          img = icondir .. "defaultpfp.svg"
        end
      	profile_picture:set_image( img )
      end
    )
  end
  ...
  local update_user_name = function()
    awful.spawn.easy_async_with_shell(
      usr.scripts .. "pfp.sh 'userName' '" .. usr.namestyle .. "'",
      function(stdout)
        if stdout:gsub("\n", "") == "Rick Astley" then
          profile_picture:set_image(
          	  usr.root .. "src/assets/userpfp/" .. "rickastley.jpg")
        end
  ...
  local suspend_command = function()
    awful.spawn("systemctl suspend")	-- FIXME
