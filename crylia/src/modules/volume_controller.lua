...
local icondir = usr.icon_dir .. 'audio/'
...
return function(s)
  ...
  local function create_device(name, node, sink)
    ...
    if sink == true then
      device:connect_signal(
        "button::press",
        function()
          awful.spawn.spawn( usr.scripts .. "vol.sh set_sink " .. node)
      ...
      device:connect_signal(
        "button::press",
        function()
          awful.spawn.spawn( usr.scripts .. "mic.sh set_source " .. node)
  ...
  awful.spawn.with_line_callback(
    [[dash -c "LC_ALL=C pactl subscribe | grep --line-buffered 'on server'"]],
    {
  ...
  local function get_mic_volume()
    awful.spawn.easy_async_with_shell(
      usr.scripts .. "mic.sh volume",
      function(stdout)
        local vtxt = stdout:gsub("%%", ""):gsub("\n", "")
        local volume = tonumber( vtxt:match("%d+"))
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider:set_value( volume )
        local svg
        if volume > 0 then
           svg = "microphone.svg"
        else
           svg = "microphone-off.svg"
        end
        volume_controller:get_children_by_id("mic_volume_margin")[1].icon:set_image(
        	gears.color.recolor_image(icondir .. svg, color["LightBlue200"] ))
     end		-- end qnon cb fn
    )			-- end spawm()
  end			-- end get_mic_volume()
  ...
  awesome.connect_signal(
    ...
    function(volume)
      ...
      volume_controller:get_children_by_id("mic_volume_margin")[1]
		.mic_volume.icon:set_image(gears.color.recolor_image(
			icondir .. svg, color["LightBlue200"] ))
    end
  )
