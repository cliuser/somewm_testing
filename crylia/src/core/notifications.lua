...
--	stub out 4.4 naughty.connect_signal() function for stock 4.3 awesome
if naughty.connect_signal == nil then
	naughty.connect_siganl = function( tag, cb ) end
end

-- local icondir = awful.util.getdir("config") .. "src/assets/icons/notifications/"
local usr = require( 'src.theme.user_variables' )
local icondir = usr.root .. "src/assets/icons/notifications/"
...
naughty.connect_signal(
  ...
  function(n)
    if n.urgency == "critical" then
      n.title = string.format("<span foreground='%s' font='%s'>%s</span>",
        color["RedA200"], usr.font.extrabold, n.title) or ""
      ...
      n.title = string.format("<span foreground='%s' font='Ts'>%s</span>",
        color["Pink200"], usr.font.extrabold, n.title) or ""
...
--[[naughty.notification {
  ...
  icon     = root .. "src/assets/userpfp/crylia.png",
  urgency  = "normal",
  timeout  = 1,
  actions  = {
    naughty.action {
