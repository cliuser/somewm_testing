...
local icon_dir = require('src.theme.user_variables').icon_dir .. 'cpu/'
...
return function(widget, clock_mode)
  ...
  watch(
      -- TODO: cpu temp is in degC, so cvt to degF.
    [[ "$( lsmod | fgrep -q vboxguest && echo 'Tctl 83.4' || sensors) | /awk '/Tctl/{print $2}'" ]],
    3,
    function(_, stdout)
  ...
  watch(
    [[ "/usr/bin/awk '/MHz/{print int($4)}' /proc/cpuinfo" ]],
    3,
    function(_, stdout)
      local cpu_freq = {}
