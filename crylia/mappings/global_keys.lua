...
local usr = require( 'src.theme.user_variables' )	-- added.

local modkey = usr.modkey
...
return gears.table.join(
  ...
  awful.key(
    ...
    "#107",
    function()
      awful.spawn(usr.screenshot_program)
    end,
        ...
        function(stdout)
          if stdout then
            ...
            awful.spawn.easy_async_with_shell(
              [[
              REMOVE="]] .. stdout:gsub("\n", "") .. [[;"
              STR=$(cat ./src/assets/rules.txt)
              echo -n ${STR//$REMOVE/} > ./src/assets/rules.txt
              ]],
              function(stdout2)
                local c = mouse.screen.selected_tag:clients()
                for j, client in ipairs(c) do
