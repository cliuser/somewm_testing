...
return function(screen, programs)
        ...
        {
          {
            ...
            forced_width = size,
            forced_height = size,
            image = user_icon or Get_icon(usr.icon_theme, nil, program, class, is_steam),
  ...
  local function get_dock_elements(pr)
    ...
    for i, p in ipairs(pr) do
      dock_elements[i] = create_dock_element(p[1], p[2], p[3], p[4], p[5], usr.dock_icon_size)
    end
  ...
  local function get_fake_elements(amount)
    ...
    for i = 0, amount, 1 do
      fake_elements[i] = wibox.widget {
        bg = '00000000',
        forced_width = usr.dock_icon_size + dpi(20),
    ...
    if s == mouse.screen then
      ...
      for _, c in ipairs(s.selected_tag:clients()) do
        ...
        if not c.minimized then
          ...
          local h = c.height
          if (y + h) >= s.geometry.height - usr.dock_icon_size - 35 then
            dock.visible = false
            return
          else
            dock.visible = true
